...
int deleteVal(HashTable* head, unsigned long val){
  ...
  for(int j=head->start;j<head->cur;j++){
	...
	return 0;
      ...
      int ret=0;
      while(!getBool(ht->InnerTable[res])){
	  ret= setPtr(&ht->InnerTable[res]);
      }
      //      printf("RET: [%d]%d->%d\n", res, ret,getBool(ht->InnerTable[res]));
	return ret; 
  ...
  if(doCopy){
	...
	if(res){
      ...
      int didins=insertTableResize(head, head->start+1, &ht->InnerTable[s], tid);
	   // int didins=insertTable(head, head->start+1, getEntPtr(ht->InnerTable[s]), tid);
