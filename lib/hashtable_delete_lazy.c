...
int getBool(entry* ent){
  return ((unsigned long)ent)&metaBits;
}
int getBoolCP(entry* ent){
  return ((unsigned long)ent)&copy;
}

int getBoolDEL(entry* ent){
  return ((unsigned long)ent)&delete;
}

entry* getPtrDEL(entry * ent){
 unsigned long mask=copy;
  return (entry*)(((unsigned long)ent)&(~mask));
}
entry* getPtr(entry* ent){
  unsigned long mask=metaBits;
  return (entry*)(((unsigned long)ent)&(~mask));
}

int setPtrCP(entry** ent){
  entry* newEnt=(entry*)((unsigned long)(*ent)|copy);
  entry* exEnt= (entry*)(((unsigned long)getPtr(*ent))|getBoolDEL(*ent));
  return __atomic_compare_exchange(ent,&exEnt, &newEnt, 1, __ATOMIC_RELAXED, __ATOMIC_RELAXED);
}

int setPtrDEL(entry** ent){
  entry* newEnt=(entry*)((unsigned long)(*ent)|delete);
  entry* exEnt= (entry*)(((unsigned long)getPtr(*ent))|getBoolCP(*ent));
  //  entry* exEnt= getPtr(*ent);
  return __atomic_compare_exchange(ent,&exEnt, &newEnt, 1, __ATOMIC_RELAXED, __ATOMIC_RELAXED);
}
int unsetCP(entry** ent){
  entry* newEnt=(entry*)((unsigned long)getPtr(*ent)|getBoolDEL(*ent));
  entry* exEnt= (entry*)(((unsigned long)(*ent)));
  //  entry* exEnt= getPtr(*ent);
  return __atomic_compare_exchange(ent,&exEnt, &newEnt, 1, __ATOMIC_RELAXED, __ATOMIC_RELAXED);
}
...
int deleteVal(HashTable* head, unsigned long val){
  ...
  for(int j=head->start;j<head->cur;j++){
	...
	return 0;
      ...
      int ret=0;
      while(!getBoolDEL(ht->InnerTable[res])){
	  ret= setPtrDEL(&ht->InnerTable[res]);
      }
	return ret; 
...
double freeAll(HashTable* head, int last, int verbose){
  ...
  for(int i = head->start;i<head->cur; i++){
	...
	if(!getBool(ht->InnerTable[j])&&!ht->didCopy[j]){
    ...
    if(verbose){
      int sumB=0,sumD=0, sum1=0;;
      for(int n =0;n<ht->TableSize;n++){
	sumB+=ht->didCopy[n];
	sumD+=!!getBoolDEL(ht->InnerTable[n]);
      }
      printf("%d: %d/%d - %d/%d - %d\n", 
	     i, items[i], ht->TableSize, sumArr(ht->threadCopy, head->numThreads), sumB, sumD);
    }
    free(ht->InnerTable);
    free(ht->threadCopy);
    //    free(ht->copyBools);
...
static int lookup(HashTable* head, SubTable* ht, entry* ent, unsigned int s, int doCopy, int tid){
  ...
  if(doCopy){
    ...
    unsigned long exCpy=0, newCpy=1;
    	int res=__atomic_compare_exchange(&ht->didCopy[s],&exCpy, &newCpy, 1, __ATOMIC_RELAXED, __ATOMIC_RELAXED);
	...
	if(res){
      ...
      int didins=insertTableResize(head, head->start+1, ht->InnerTable+s, tid);
	   // int didins=insertTable(head, head->start+1, getPtr(ht->InnerTable[s]), tid);
...
int insertTableResize(HashTable* head,  int start, entry** ent, int tid){
  ...
  for(int j=start;j<head->cur;j++){
    ...
    for(int i =0; i<head->hashAttempts; i++) {
      ...
      int res=lookup(head, ht, *ent, buckets[i]%ht->TableSize, 0, tid);
      if(res==unk){ //unkown if in our not
	continue;
      }
      if(res==in){ //is in
	...
	return 0;
      ...
      entry* expected=NULL;
      if(getBoolDEL(ht->InnerTable[res])){
	expected=ht->InnerTable[res];
      }
      //      expected=getPtrDEL(ht->InnerTable[res]);
      //      if(getBoolDEL(ht->InnerTable[res])){
      //expected=getPtrDEL(ht->InnerTable[res]);
	//      }
        //    *ent=getPtrDEL(*ent);
...
createTable(HashTable* head, int tsize){
  ...
  ht->TableSize=tsize;
  ht->didCopy=(unsigned long*)calloc(tsize, sizeof(unsigned long));
